...
class Instalador
{
    ...
    private static function migracoesLeves(): void
    {
        ...
        if ($versao < 32) {
            // Mapa Territorial (PR 1): imóvel rural (dim_imovel), vínculo N:M
            // imóvel↔produtor (bridge) e talhão por safra (fato). Geometria como
            // JSON + bbox (mesmo padrão de car_imoveis; consulta espacial em PHP).
            if (!self::temTabela('dim_imovel')) {
                Database::executar(
                    'CREATE TABLE dim_imovel (
                        ...
                        situacao_car VARCHAR(30) NULL,
                        contorno MEDIUMTEXT NOT NULL,
                        contorno_simpl MEDIUMTEXT NULL,
            ...
            if (!self::temTabela('bridge_imovel_produtor')) {
                Database::executar(
            ...
            if (!self::temTabela('fato_talhao_safra')) {
                Database::executar(
                    'CREATE TABLE fato_talhao_safra (
                        ...
                        produtividade DECIMAL(10,3) NULL,
                        contorno MEDIUMTEXT NULL,
            ...
            // Seed de demonstração (só se vazio): 5 imóveis fictícios em Concórdia/SC.
            // A bridge NÃO é semeada aqui (depende de clientes que podem não existir
            // no banco já em uso) — o vínculo demo fica só no database.sql (dev).
            if (self::temTabela('dim_imovel') && (int) Database::valor('SELECT COUNT(*) FROM dim_imovel') === 0) {
                Database::executar(
            ...
            if (self::temTabela('fato_talhao_safra') && (int) Database::valor('SELECT COUNT(*) FROM fato_talhao_safra') === 0) {
                Database::executar(
            ...
            Database::executar(
                "INSERT INTO configuracoes (chave, valor) VALUES ('schema_versao', '32')
