      ...
      <div class="border-top">
        <div class="px-3 pt-2 pb-1 d-flex justify-content-between align-items-start flex-wrap gap-1 bg-light bg-opacity-50">
          <div>
            ...
            <span class="text-muted small">· total <?= numero($r['area_total'], 1) ?> ha · plantio <?= numero($r['area_plantio'], 1) ?> ha<?= $r['plantio_origem'] === 'total' ? ' (= total)' : (count($r['areas'] ?? []) > 1 ? ' (' . count($r['areas']) . ' áreas)' : '') ?><?= ($r['nao_plantio'] ?? 0) > 0 ? ' · não plantio ' . numero($r['nao_plantio'], 1) . ' ha' : '' ?><?= !empty($im['municipio']) ? ' · ' . e($im['municipio']) . (!empty($im['uf']) ? '/' . e($im['uf']) : '') : '' ?></span>
            <?php if (count($r['areas'] ?? []) > 0): ?>
              <div class="small text-muted"><i class="bi bi-layers me-1 text-success"></i>Áreas de plantio:
                <?php foreach ($r['areas'] as $ap): ?><span class="badge text-bg-light border text-dark me-1"<?= ($ap['desconto'] ?? 0) > 0 ? ' title="Desenho: ' . numero($ap['area_gps'], 1) . ' ha − não plantio ' . numero($ap['desconto'], 1) . ' ha"' : '' ?>><?= e($ap['nome']) ?> · <?= numero($ap['area_liquida'] ?? $ap['area_gps'], 1) ?> ha</span><?php endforeach; ?>
              </div>
            <?php endif; ?>
        ...
        <?php
          ...
          $linhasTalhao = [];
          foreach ($grupos as $g) { if ($g['talhoes']) { $apL = $liquidaPorArea[(int) $g['area']['id']] ?? null; $linhasTalhao[] = ['cab' => '🌱 ' . $g['area']['nome'] . ' · ' . numero((float) ($apL['area_liquida'] ?? $g['area']['area_gps']), 1) . ' ha', 'itens' => $g['talhoes']]; } }
