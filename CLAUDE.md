...
- **Offline básico (Fase 1)**: service worker cacheia o app/assets (assets *cache-first*; páginas *rede-primeiro* → a última versão vista abre offline). Visitas/fotos criadas sem sinal entram em fila local no IndexedDB (`crm_coperdia` › `fila_visitas`) e sincronizam ao reconectar (evento `online` + ~1,5 s após cada load); a fila reenvia para a rota `visitas/salvar`, que recalcula tudo no servidor. **Hoje só a criação de Visitas funciona offline** e não há snapshot da carteira em IndexedDB (leitura offline = só telas já visitadas em cache). Plano de expansão em `docs/OFFLINE_AMPLIADO.md`.
